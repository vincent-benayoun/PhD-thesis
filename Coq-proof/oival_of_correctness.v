...
Theorem oival_of_completeness :
  forall (c:env) (e:expr) (v:val),
    val_of c e v
    -> (exists (uu:oitval), oival_of (env_to_oitenv c) e uu
                            /\ oitval_to_val uu = v
       ).
Proof.
  intros c e v H.
  ...
  (* case Ident *)
  exists(val_to_oitval v).
  split; try (symmetry; apply val_to_oival_to_val).
  apply OIVal_of_Ident.

  induction c.
  inversion H.
  simpl; simpl in H.
  induction (beq_identifier i x); try inversion H; auto.

  simpl.

  (* case Lambda *)
  exists(OIV nil (OIV_ nil (OIV_Closure x e (env_to_oienv c)))).
  split.
  apply OIVal_of_Lambda.
  repeat f_equal; auto.
  rewrite oitenv_to_oienv_env_to_oienv; reflexivity.
  simpl.
  f_equal.
  rewrite env_to_oienv_to_env; reflexivity.

  (* case Rec *)
  exists(OIV nil (OIV_ nil (OIV_Rec_Closure f x e (env_to_oienv c)))).
  split.
  apply OIVal_of_Rec.
  repeat f_equal; auto.
  rewrite oitenv_to_oienv_env_to_oienv; reflexivity.
  ...
  (* case Apply *)
  inversion_clear IHval_of1 as [uu1 IHoival_of1].
  inversion_clear IHval_of2 as [uu2 IHoival_of2].
  inversion_clear IHval_of3 as [uu IHoival_of].

  inversion_clear IHoival_of1 as [IH_oival_of_e1 IH_oival_of_e1_rel].
  inversion_clear IHoival_of2 as [IH_oival_of_e2 IH_oival_of_e2_rel].
  inversion_clear IHoival_of as [IH_oival_of_e IH_oival_of_e_rel].
  
  induction uu1 as [td1 u1].
  induction u1 as [d1 v1].
  induction uu2 as [td2 u2] eqn:H_uu2.
  rename v into v'.
  induction uu as [td u].
  induction u as [d v].

  assert(HH:=exists_deps_spec_Apply d1 uu2).
  inversion_clear HH as [td' HHH].
  inversion_clear HHH as [d' HH].

(*
  exists(OIV (conc_oitdeps td1 (conc_oitdeps td2 (conc_oitdeps td td'))) (OIV_ (conc_oideps d' d) v)).
  split; try apply IHoival_of.

  remember (OIV (conc_oitdeps td1 (conc_oitdeps td2 (conc_oitdeps td td'))) (OIV_ (conc_oideps d' d) v)) as uu.

  apply (OIVal_of_Apply (env_to_oitenv c) _ e1 e2 e x uu2 uu u2 d _ _ _ td2 td nil v).
*)
  
Admitted. (* oival_of_completeness *)